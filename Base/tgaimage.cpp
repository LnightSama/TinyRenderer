...
bool TGAImage::read_jpg_file(const std::string filename) {
	//GdiplusStartupInput gdiplusstartupinput;
	//ULONG_PTR gdiplustoken;
	//GdiplusStartup(&gdiplustoken, &gdiplusstartupinput, NULL);

	//wstring infilename(L"" + filename);
...
void TGAImage::flip_clockwise90() {
	int h_half = h >> 1;
    int w_half = w >> 1;
	for (int i = 0; i < w_half; i++)
		for (int j = 0; j < h_half; j++)
            for (int b = 0; b < bpp; b++) {
				std::swap(data[(i + j * w) * bpp + b], data[(j + (h - 1 - i) * w) * bpp + b]);
				std::swap(data[(i + j * w) * bpp + b], data[(w - 1 - i + (h - 1 - j) * w) * bpp + b]);
				std::swap(data[(i + j * w) * bpp + b], data[(h - 1 - j + (i) * w) * bpp + b]);
            }
}

void TGAImage::flip_antiClockwise90() {
	int h_half = h >> 1;
	int w_half = w >> 1;
	for (int i = 0; i < w_half; i++)
		for (int j = 0; j < h_half; j++)
